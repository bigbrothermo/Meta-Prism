...
__global__ void gpu_Calc_sim(CompData *cd, Abd *v_Abd_1, Abd *v_Abd_2, float *results);
...
int gpu_compare::act()
{
    ...
    gpu_Calc_sim<<<this->count, 1>>>(this->g_compData, this->g_abd1, this->g_abd2, this->g_results);
    this->state=3;
    return 0;
}
...
__global__ void gpu_Calc_sim(CompData *cd, Abd *v_Abd_1, Abd *v_Abd_2, float *results)
{
    ...
    v_Abd_2=(Abd *)v_Abd_2 +blockIdx.x;
