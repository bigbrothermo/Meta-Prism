...
private:
    ...
    int version,compFlag,minGridSize,bestBlockSize;
};
...
gpu_compare::gpu_compare()
{
    ...
    cudaOccupancyMaxPotentialBlockSize( &this->minGridSize, &this->bestBlockSize, gpu_Calc_sim());
...
int gpu_compare::act()
{
    ...
    blocks=this->count/this->bestBlockSize+1;
    gpu_Calc_sim<<<blocks, this->bestBlockSize>>>(this->g_compData, this->g_abd1, this->g_abd2, this->g_results,this->count);
    this->state=3;
    return 0;
}
