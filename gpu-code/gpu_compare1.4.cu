...
__global__ void gpu_Calc_sim(CompData *cd, Abd *v_Abd_1,
                             float *g_hashmap, unsigned int * g_abdIndex, float *results);
class gpu_compare
{
public:
    gpu_compare();
    int sendData(CompData * cd,Abd * abd1,
                 Abd * abd2,int count, int version = 2);
...
gpu_compare::gpu_compare()
{
    ...
    cudaHostAlloc( (void**)&this->results, this->maxCount*sizeof(float),
...
int gpu_compare::sendData(CompData * cd,Abd *abd1, Abd *abd2,int count,int version)
{
    ...
    */
    ...
    this->getInfo();
    //time
    if (totalMem<sizeof(float)*(LeafN*(count+1)+count)*2)
    {
    ...
    if(this->count*1.1>this->maxCount)
    {
        this->maxCount=this->count*1.2;
        cudaFreeHost(this->results);
        cudaHostAlloc( (void**)&this->results, this->maxCount*sizeof(float),
                       cudaHostAllocDefault);
    }
    ...
    cudaMemcpy(this->g_abdIndex,this->abdIndex,sizeof(unsigned int)*count,
...
{
    HashNode *hashmap,hBuffer;
    Abd *abd2;
    string nameBuffer;
    unsigned int offset,offset2,*abdIndex;
    int crashTime,flag,timer,hashSize;
    unsigned int queryTime;
    hashmap=this->hashmap;
    ...
    {
        offset=BKDRHash(abd2[i].name);
        offset2=offset;
        flag=1;
        for (int t = 0 ;t < 5||flag ==1; t++) {
            offset2=(offset2+t*t)%hashSize;
            if(hashmap[offset2].time==0){
                offset=offset2;
                this->usedHash++;
                break;
            }
            if(hashmap[offset2].name==abd2[i].name){
                hashmap[offset2].time=queryTime;
                abdIndex[i]=offset2;
                timer++;
                flag=2;
                break;
        ...
        if(flag!=2){
            abdIndex[i]=offset;
            cudaMemcpy(this->g_hashmap+offset,abd2[i].data,
                       sizeof(float) * LeafN,cudaMemcpyHostToDevice);
            hashmap[offset].name=abd2[i].name;
            hashmap[offset].time=queryTime;
        }
    }
    cout<<"cache missing rate:"<<1-(float)timer/count<<endl;
}
int gpu_compare::act()
{
    if(this->state!=2)
    ...
    gpu_Calc_sim<<<this->count, 1>>>(this->g_compData,
            this->g_abd1, this->g_hashmap,
            this->g_abdIndex, this->g_results);
    this->state=3;
    return 0;
}
int gpu_compare::getResult(float &*putResult)
{
    ...
    cudaMemcpy(this->results, this->g_results, sizeof(float) * count,cudaMemcpyDeviceToHost);
    gettimeofday(&tv_end,NULL);
    double copyBackTime = double(tv_end.tv_sec-tv_begin.tv_sec)*1000000
    ...
    cudaFree(this->g_compData);
    cudaFree(this->g_abdIndex);
    cudaFree(this->g_results);
    //delete []this->abd2,[]this->hashmap,[]this->abdIndex,[]this->abd1;
    return 0;
}
__global__ void gpu_Calc_sim(CompData *cd, Abd *v_Abd_1, float *g_hashmap,
                             unsigned int * g_abdIndex, float *results)
{
    //process memory data
    float *Dist_1;
    float *Dist_2;
