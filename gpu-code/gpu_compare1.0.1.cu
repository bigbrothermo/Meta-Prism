...
int gpu_compare::act()
{
    ...
    wrapSize=this->deviceProp.warpSize;
    i=this->count/(this->deviceProp.multiProcessorCount*wrapSize)+1;
    blocks=this->countCore;
    wrapCount=wrapSize*i;
    gpu_Calc_sim<<<blocks, wrapCount>>>(this->g_compData, this->g_abd1, this->g_abd2, this->g_results,this->count);
    this->state=3;
    return 0;
}
